...
<template>
    <div :class="place.current.is_day === 1 ? 'bg-day' : 'bg-night'" class="text-white p-10 rounded-3xl shadow-lg gap-6 relative overflow-hidden">
        <!-- Location & time -->
        <div class="mb-2 flex justify-between items-center">
            ...
            <div class="flex items-center justify-center gap-2 text-3xl">
                ...
                <h1>
                    {{ new Date(place.location.localtime).getHours() }}.{{ new Date(place.location.localtime).getMinutes()
                    }}
                </h1>
            </div>
        </div>
        ...
        <div class="text-center flex-1">
            ...
            <h1 class="text-9xl font-medium mb-2 -mr-6">{{ Math.round(place.current.temp_c) }}&deg;</h1>
            <p class="text-2xl capitalize">{{ place.current.condition.text }}</p>
        </div>
